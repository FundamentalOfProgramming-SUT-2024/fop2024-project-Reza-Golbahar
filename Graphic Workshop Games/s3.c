...
{
    for (int i = 0; i < 100; i++)
        array[i][0] = array[i][1] = array[i][2] = k;
}
...
{
    for (int i = 0; i < 100; i++)
        if (cloud_coordinate[state][i][0] != -1)
            map[cloud_coordinate[state][i][0]][cloud_coordinate[state][i][1]] = ' ';
    for (int i = 0; i < 100; i++)
        cloud_coordinate[state][i][0] = cloud_coordinate[state][i][1] = -1;
    int iterator = 0;
    for (int i = 0; i < 6; i++)
        for (int j = 0; j < 29; j++)
            if (cloud[state][i][j] != ' ')
            {
                ...
                if (y + j < 0 || y + j > width - 1)
                    cloud_coordinate[state][iterator][0] = -1;
                iterator++;
            }
    if (state)
        cloud_x[state] -= 2;
    ...
    for (int i = 0; i < 100; i++)
        if (cloud_coordinate[state][i][0] != -1)
            map[cloud_coordinate[state][i][0]][cloud_coordinate[state][i][1]] = (char)cloud_coordinate[state][i][2];
}
void set_cactus_in_map(int x, int y, int state)
{
    // clear previous
    for (int i = 0; i < 100; i++)
        if (cactus_coordinate[state][i][0] != -1)
            map[cactus_coordinate[state][i][0]][cactus_coordinate[state][i][1]] = ' ';
    // set new cactus
    for (int i = 0; i < 100; i++)
        cactus_coordinate[state][i][0] = cactus_coordinate[state][i][1] = -1;
    int iterator = 0;
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 12; j++)
        {
            if (cactus[state][i][j] != ' ')
            {
                ...
                if (y + j < 0 || y + j > width - 1)
                    cactus_coordinate[state][iterator][0] = -1;
                iterator++;
            }
        }
    }
    ...
    for (int i = 0; i < 100; i++)
        if (cactus_coordinate[state][i][0] != -1)
            map[cactus_coordinate[state][i][0]][cactus_coordinate[state][i][1]] = (char)cactus_coordinate[state][i][2];
}
void lost()
{
...
void set_dinasour_in_map(int x, int y, int state)
{
    ...
    for (int i = 0; i < 100; i++)
        if (dinasour_coordinate[i][0] != -1)
            map[dinasour_coordinate[i][0]][dinasour_coordinate[i][1]] = ' ';
    // set new dinasour
    ...
    for (int i = 0; i < 100;i++)
        if (dinasour_coordinate[i][0] != -1)
            map[dinasour_coordinate[i][0]][dinasour_coordinate[i][1]] = (char)dinasour_coordinate[i][2];
...
int main()
{
    ...
    while (game_state)
    {
        ...
        {
            if ((ch == KEY_UP || ch == (int)' ') && (is_jumping == 0))
            {
                is_jumping = 1;
                jump();
            }
